...
/*
Vue cho phép chúng ta define filters có thể dùng để apply các common text formatting. Filter được dùng ở 2 chỗ: mustache interpolations + `v-bind` expression.
- Có thể define filter trong component hoặc defind globally trước khi creating Vue instance
- Filters cũng có thể nhận tham số, tham khảo https://vuejs.org/v2/guide/filters.html
*/
...
Vue.component('post-item', {
  ...
  // Định nghĩa filter trong component
  filters: {
    capitalizeFirstLetter: function(text) {
      const textArr = text.split(' ').map(t => t.charAt(0).toUpperCase() + t.slice(1));
      return textArr.join(' ');
    }
  },
  computed: {
  ...
  template: `
    <div class="post-item">
      <h5>{{ post.title | capitalizeFirstLetter }}</h5>
      <button
